...
import { setErrorModal } from '../../../redux/cart/slice';

const ModalCartError: React.FC = () => {
  const dispatch = useDispatch();

  const toggleSwitchError = () => {
    ...
    <div className="modal">
      <div className="modal__bg">
        <div className="modal__top">
          ...
          <div className="modal__body">Товар уже додано у кошик</div>
        </div>
        <div className="modal__buttons button">
          <button className="button__left" onClick={toggleSwitchError}>
            <Link to={`/product/${id}`}>Продовжити покупки</Link>
          </button>
          <button className="button__right" onClick={toggleSwitchError}>
            <Link to={`/cart`}>Перейти до кошику</Link>
          </button>
        </div>
      </div>
    </div>
